...
@Autonomous(name = "Albastru completa brigde V2")
@Disabled
public class AutonomousBlueNEW extends LinearOpMode {
    ...
    void rotateFundation() {
        ...
        robot.runUsingEncoders(-835, 0.7, 5);//1700

        robot.rotateConstantSpeed(70, 0.7, 5);

        robot.runUsingEncoders(500, 0.7, 5);//500
...
//        sleep(500);
        ...
        robot.runUsingEncoders(-100, 1, 3);

        robot.strafe(400, 0.8, 3);

//        double correction = robot.checkDirection();
//        robot.rotate((int) correction, 0.2, 2);

        robot.runUsingEncoders(-1725, 1, 5);//3000
...
//        }
        ...
        robot.runUsingEncodersCorrection(150, 0.8, 3);
        robot.ghearaDreapta.setPosition(1);
        sleep(600);
        robot.ridicareBratDreapta.setPower(-0.85);
        sleep(700);
        robot.ridicareBratDreapta.setPower(0);
        robot.runUsingEncoders(-325, 0.9, 3);
        robot.rotate(88, 1, 2);
        // robot.runUsingEncoders(7300, 1, 9);
        robot.runUsingEncodersCorrection(robot.conversieCmToTick(1000), 1, 9);
    ...
    void skystoneRight() {
        ...
        telemetry.update();
        robot.runUsingEncoders(150,0.5, 3);
        robot.ghearaDreapta.setPosition(1);
        sleep(600);
        robot.ridicareBratDreapta.setPower(-0.85);
        sleep(700);
        ...
        robot.runUsingEncodersCorrection(robot.conversieCmToTick(1115), 1, 9);
        while (robot.senzorDistanta.getDistance(DistanceUnit.MM) >= 450) {   //350
            robot.forward(0.2);
            telemetry.addData("mm", "%.2f mm", robot.senzorDistanta.getDistance(DistanceUnit.MM));
            telemetry.update();
        }
...
//            telemetry.update();
        ...
        robot.runUsingEncoders(-400, 0.8, 3);
        robot.rotate(88, 1, 2);
        robot.runUsingEncodersCorrection(3950, 1, 9);//7900
        while (robot.senzorDistanta.getDistance(DistanceUnit.MM) >= 350) {
            robot.forward(0.3);
//            telemetry.addData("mm", "%.2f mm", robot.senzorDistanta.getDistance(DistanceUnit.MM));
//            telemetry.update();
    ...
    public void runOpMode() throws InterruptedException {
        ...
        if (opModeIsActive()) {
            ...
            robot.runUsingEncodersCorrection(robot.conversieCmToTick(325), 0.9, 4);
