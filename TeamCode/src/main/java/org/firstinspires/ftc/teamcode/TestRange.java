...
@Autonomous(name = "senzorDistanta")

public class TestRange extends LinearOpMode {
...
//    void mergeSpate(){
//        robot.runUsingEncoders(-1500,0.4,5);
//    }
    ...
    void skystoneLeft() {
        ...
        telemetry.update();
        robot.strafe(-1050, 0.65, 3);//1000
        double correction = robot.checkDirection();
        robot.rotate((int) correction, 0.2, 2);

        while (robot.senzorDistanta.getDistance(DistanceUnit.MM) <= 65) {
            robot.forward(-0.15);
            telemetry.addData("cm", "%.2f cm", robot.senzorDistanta.getDistance(DistanceUnit.CM));
            telemetry.update();
        }
    ...
    void skystoneRight() {
        ...
        robot.runUsingEncoders(-600, 0.5, 3);
        robot.rotate(78, 0.7, 2);
        robot.runUsingEncoders(7500, 0.8, 9);
        while (robot.senzorDistanta.getDistance(DistanceUnit.MM) >= 350) {
            robot.forward(0.3);
            telemetry.addData("cm", "%.2f cm", robot.senzorDistanta.getDistance(DistanceUnit.CM));
            telemetry.update();
    ...
    void noSkystone() {
        ...
        while (robot.senzorDistanta.getDistance(DistanceUnit.CM) <= 65) {
            robot.forward(-0.15);
            telemetry.addData("cm", "%.2f cm", robot.senzorDistanta.getDistance(DistanceUnit.CM));
            telemetry.update();
        }
...
//
//            robot.runUsingEncoders(2050, 0.6, 4);//2550
//
...
//            }
//
////            while (robot.senzorDistanta.getDistance(DistanceUnit.CM) >= 10) {
////                robot.forward(0.2);
////                telemetry.addData("raw ultrasonic", robot.senzorDistanta.rawUltrasonic());
////                telemetry.addData("raw optical", robot.senzorDistanta.rawOptical());
////                telemetry.addData("cm optical", "%.2f cm", robot.senzorDistanta.cmOptical());
////                telemetry.addData("cm", "%.2f cm", robot.senzorDistanta.getDistance(DistanceUnit.CM));
////                telemetry.update();
////            }
//
